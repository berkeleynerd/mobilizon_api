...
class RateLimitProfile {
  ...
  static const standard = RateLimitProfile(
    ...
    authenticationDelay: Duration(milliseconds: 1500), // Increased for rate limiting
    mediaDelay: Duration(seconds: 2),
    profileUpdateDelay: Duration(milliseconds: 600),
    testStartDelay: Duration(milliseconds: 500), // Increased pre-test delay
    batchOperationDelay: Duration(milliseconds: 300),
  );
  ...
  Duration getDelayFor(RateLimitType type) {
    switch (type) {
      ...
      case RateLimitType.passwordReset:
        return authenticationDelay * 2; // Double delay for password reset
